function S = deconv_dark_chanel(Im, kernel, lambda, kappa)
%%
% Image restoration with L0 regularized intensity and gradient prior
% The objective function:
% S = argmin ||I*k - B||^2 + \lambda |D(I)|_0 
%% Input:
% @Im: Blurred image
% @kernel: blur kernel
...
%         Normin2 = Normin2 + [v(end,:,:) - v(1, :,:); -diff(v,1,1)];
        %
        FS = (Normin1 + mybeta_pixel*fft2(u))./Denormin;
        S = real(ifft2(FS));
        %%
        beta = beta*kappa;
%         if wei_grad==0
